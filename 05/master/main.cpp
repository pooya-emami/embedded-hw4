...
std::string ask_slave_latest(const std::string &ip, int port,
...
std::string search_latest(const std::string &db_file,
                          const std::string &type,
                          const std::string &id) {
    sqlite3 *db;
    sqlite3_stmt *stmt;
...
std::string search_history(const std::string &db_file,
                           const std::string &type,
                           const std::string &id,
                           const std::string &date) {
    sqlite3 *db;
...
std::string resolve_sensor(const std::string &sensor_type, const std::string &sensor_id) {
    ...
    if (answer.empty()) {
        answer = search_latest(cfg.db, sensor_type, sensor_id);
        if (!answer.empty()) {
    ...
    if (answer.empty()) {
        ...
        slave_answer = ask_slave_latest(cfg.slave1_ip, cfg.slave1_port, sensor_type, sensor_id);
        ...
        if (answer.empty()) {
            slave_answer = ask_slave_latest(cfg.slave2_ip, cfg.slave2_port, sensor_type, sensor_id);
            if (!slave_answer.empty() && slave_answer.find("\"error\"") == std::string::npos) {
                if (slave_answer.find("\"source\":\"slave_cache\"") != std::string::npos)
                    source = "slave_cache";
                else
    ...
    if (mg_strcmp(msg->uri, mg_str("/history")) == 0) {
        ...
        if (has_data(answer)) {
            ...
            mg_http_reply(c, 200,
                          ...
                          "\"date\":\"%s\",\"values\":[]}",
                          sensor_type.c_str(), sensor_id.c_str(), sensor_date.c_str());
        }
        return;
    }
